    ...
    return (
        <section className="relative w-full h-[100dvh] md:h-auto md:min-h-screen flex items-center justify-center bg-background md:py-20 px-4 md:px-6 transition-colors duration-300 overflow-hidden">
            ...
            <div className="absolute inset-0 z-0 md:hidden">
                ...
                />
                {/* Gradient diturunkan menjadi 50% saja agar bagian atas layar lebih bersih */}
                <div className="absolute bottom-0 left-0 right-0 h-[50%] bg-gradient-to-t from-background via-background/95 to-transparent"></div>
            </div>
            ...
            <div className="relative z-10 max-w-6xl w-full h-full md:h-auto flex flex-col justify-end pb-8 md:pb-0 md:grid md:grid-cols-2 gap-12 items-center mt-0 md:mt-16">
                ...
                {/* Text Content - Ukuran diperkecil ekstrem untuk mobile, dipertahankan normal untuk md ke atas */}
                <div className="space-y-3 md:space-y-8 text-center md:text-left">
                    <div className="space-y-1 md:space-y-4">
                        ...
                        <h1 className="text-3xl md:text-6xl font-bold text-content tracking-tight capitalize transition-colors">
                            {personal.name}
                        </h1>
                        <p className="text-sm md:text-xl text-content-muted leading-relaxed max-w-lg mx-auto md:mx-0 transition-colors">
                            {personal.tagline}
                        </p>
                    </div>
                    ...
                    <div className="flex flex-wrap items-center justify-center md:justify-start gap-2.5 md:gap-4">
                        ...
                        <button className="flex items-center gap-1.5 md:gap-2 bg-surface text-content px-4 py-2 md:px-7 md:py-3.5 rounded-lg md:rounded-xl border border-border hover:bg-background transition-colors shadow-sm text-sm md:text-base font-medium">
                            <Download className="w-4 h-4 md:w-5 md:h-5" /> {cta.secondary}
                        </button>
                    </div>

                    {/* Social Links - Ikon dan margin diperkecil di mobile */}
                    <div className="flex items-center justify-center md:justify-start gap-4 md:gap-5 pt-1 md:pt-4 text-content-muted">
