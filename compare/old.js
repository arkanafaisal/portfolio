...
export default function FeaturedProjectsSection() {
    ...
    useEffect(() => {
        if (selectedProject && detailsRef.current) {
            setTimeout(() => {
                // scroll-mt diperkecil untuk mobile agar jarak atas tidak terlalu jauh
        ...
        <section className="py-12 md:py-24 px-6 bg-background transition-colors duration-300">
            <div className="max-w-6xl mx-auto space-y-8 md:space-y-12">
                ...
                {selectedProject && (
                    <div 
                        ref={detailsRef}
                        /* Padding diperkecil (p-5), margin top disesuaikan, dan scroll margin disetel agar nyaman di mobile */
                    ...
                    >
                        {/* Menggunakan flex-col untuk mobile, dan grid untuk Desktop */}
                        <div className="flex flex-col lg:grid lg:grid-cols-2 gap-5 md:gap-8 items-center w-full">
                            
                            {/* Detail Image/Mockup - Landscape 
                                order-1 membuatnya berada di paling atas pada mobile, order-2 mengembalikannya ke sisi kanan pada PC 
                            */}
                            <div className="order-1 lg:order-2 relative w-full rounded-xl overflow-hidden border border-border shadow-md h-48 sm:h-56 md:h-80 lg:h-96">
                                <img 
                            ...
                            {/* Detail Text 
                                order-2 untuk mobile (di bawah gambar), order-1 untuk PC (di sebelah kiri gambar)
                            */}
                            <div className="order-2 lg:order-1 w-full space-y-4 md:space-y-6">
                                ...
                                {/* Action Buttons - Dibuat full width (memanjang) secara vertikal di mobile terkecil, menyamping di layar agak besar */}
